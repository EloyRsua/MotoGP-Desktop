class Ciudad {
    ...
    procesarJSONCarrera(datos) {
        ...
        article.append("<h3>Meteorología del día de la carrera</h3>");
        ...
        // Recorrer datos horarios y mostrarlos con encabezados para cada hora
        for (let i = 0; i < hourly.time.length; i++) {
            ...
            if (hora >= 11 && hora <= 17) {
                // Crear un párrafo destacado para la hora
                const pHora = $("<p></p>").html(`Hora: ${horaCompleta}`);
                article.append(pHora);
                
                // Crear lista con los datos meteorológicos
                const ul = $("<ul></ul>");
                ul.append($("<li></li>").text(`Temperatura: ${hourly.temperature_2m[i]}°C`));
                ul.append($("<li></li>").text(`Sensación térmica: ${hourly.apparent_temperature[i]}°C`));
                ul.append($("<li></li>").text(`Lluvia: ${hourly.rain[i]}mm`));
                ul.append($("<li></li>").text(`Humedad: ${hourly.relative_humidity_2m[i]}%`));
                ul.append($("<li></li>").text(`Velocidad viento: ${hourly.wind_speed_10m[i]}km/h`));
                ul.append($("<li></li>").text(`Dirección viento: ${hourly.wind_direction_10m[i]}°`));
    ...
    procesarJSONEntrenos(datos) {
        ...
        // Mostrar resultados
        const article = $("<article></article>");
        article.append("<h3>Meteorología de los días de entrenamientos</h3>");

        for (const fecha in medias) {
            const media = medias[fecha];
            
            const pFecha = $("<p></p>").html(`Día: ${fecha}`);
            article.append(pFecha);
            
            const ul = $("<ul></ul>");
            ul.append($("<li></li>").text(`Temperatura media: ${media.temperatura}°C`));
            ul.append($("<li></li>").text(`Lluvia media: ${media.lluvia}mm`));
            ul.append($("<li></li>").text(`Velocidad viento media: ${media.velocidadViento}km/h`));
            ul.append($("<li></li>").text(`Humedad media: ${media.humedad}%`));
